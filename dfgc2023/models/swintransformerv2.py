...
from mindcv.models.helpers import _ntuple, load_pretrained
from mindcv.models.layers import DropPath, Identity
...
class WindowCosineAttention(nn.Cell):
    def __init__(
        ...
        if qkv_bias:
            self.q_bias = Parameter(ops.zeros(dim), requires_grad=True)
            self.k_bias = Parameter(ops.zeros(dim), requires_grad=False)
            self.v_bias = Parameter(ops.zeros(dim), requires_grad=True)
        else:
            self.q_bias = None
            self.k_bias = None
            self.v_bias = None
    ...
    def construct(self, x: Tensor, mask=None) -> Tensor:
        ...
        qkv = self.qkv(x)
        qkv = ops.reshape(qkv, (-1, qkv.shape[-1]))
        if self.q_bias is not None:
            qkv_bias = ops.cat((self.q_bias, self.k_bias, self.v_bias))
            qkv = ops.bias_add(qkv, qkv_bias)
        
        qkv = ops.reshape(qkv, (B_,N, qkv.shape[-1]))
        qkv = ops.reshape(qkv,(B_, N, 3, self.num_heads, -1))
        qkv = ops.transpose(qkv, (2, 0, 3, 1, 4))
        q, k, v = ops.unbind(qkv, dim=0)
        k = ops.transpose(k, (0, 1, 3, 2))
        
        attn = self.matmul(self.Normalize(q), self.Normalize(k))
